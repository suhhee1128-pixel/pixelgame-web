...
export default function ItemTab() {
  ...
  const [status, setStatus] = useState('Ready to generate item...');
  const [promptPreview, setPromptPreview] = useState('Enter item description and style preferences to see the generated prompt...');
  ...
  useEffect(() => {
    updatePromptPreview();
  }, [description, artStyle, mood, colorPalette, lineStyle, composition, additionalNotes]);
  ...
  const updatePromptPreview = async () => {
    if (!description.trim()) {
      setPromptPreview('Enter item description and style preferences to see the generated prompt...');
      return;
    }
  ...
  const generateItem = async () => {
    if (!description.trim()) {
      setStatus('Please enter an item description');
      return;
    }

    setLoading(true);
    setStatus('Generating item...');
    ...
    try {
      ...
      if (data.success) {
        ...
        setStatus('✅ Item generated successfully!');
      } else {
        setStatus(`❌ Error: ${data.error}`);
      }
    } catch (error: any) {
      setStatus(`❌ Error generating item: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">🖼️ Item Reference (Optional)</label>
          <input
            type="file"
            accept="image/*"

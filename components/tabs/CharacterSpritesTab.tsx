...
export default function CharacterSpritesTab() {
  ...
  const [status, setStatus] = useState('Ready to generate sprites...');
  ...
  useEffect(() => {
    updatePromptPreview();
  }, [description, actionsText, artStyle, mood, colorPalette, characterStyle, lineStyle, composition, additionalNotes]);
  ...
  const updatePromptPreview = async () => {
    if (!description.trim() || !actionsText.trim()) {
      setPromptPreview('Enter character description and actions to see the generated prompt...');
      return;
    }
  ...
  const generateSprites = async () => {
    ...
    try {
      ...
      formData.append('actions_text', actionsText);
      ...
      const data = await response.json();
      if (data.success) {
        setGeneratedSprites(data.sprites.map((s: any) => s.image_url));
        setStatus(`✅ Generated ${data.sprites.length} sprites successfully!`);
      } else {
        setStatus(`❌ Error: ${data.error}`);
      }
    } catch (error: any) {
      setStatus(`❌ Error generating sprites: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium mb-2">Actions (comma-separated)</label>
          <textarea
            value={actionsText}
